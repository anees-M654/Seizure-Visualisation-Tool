...
Methodology:
Evolutionary development – pending 
Since the heatmap can be created early on changes to zoom or colour scales can be implemented throughout the process.
This system will handle different spreadsheet structures( optional req) this approach will allow us to continue to update and refine the data through multiple versions.
Reduction of risk as the core requirements can be implemented first then they optional requirements in the next iteration.
Identifying requirements, needs or constraints:
-	Data must be able to be imported from both CSV and XLS format
...
-	
-	(Audit Logging) – If deployed on server, system must maintain logs of user activity for accountability and security.
-	
-	Tool should dynamically accept data with additional column and structure from datasets without changing code.
# Y&H ROCU Seizure Visualisation Tool
